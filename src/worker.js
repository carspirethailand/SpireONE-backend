        ...
        return await guarded('user', async (actor) => {
          ...
          if (!b.refresh) {
            ...
            if (hit && Date.now() - hit.t < TTL) {
              try {
                return json({ items: JSON.parse(hit.payload), cachedAt: hit.t, cached: true, apps });
          ...
          const items = await getSpares(env, { car, apps, needs, lang: b.lang || 'th' });
