...
async function getActor(request, env) {
  ...
  let row = null;
  try {
    row = await env.DB.prepare('SELECT role, banned FROM users WHERE uid = ?').bind(payload.sub).first();
  } catch (e) {
    // banned column not migrated yet — degrade gracefully (run migration 0003)
    try { row = await env.DB.prepare('SELECT role FROM users WHERE uid = ?').bind(payload.sub).first(); } catch (e2) {}
  }
  return {
    payload, email,
 ...
 * Resilient: retries once on 429/503, then tries GEMINI_FALLBACK_MODEL.
 * Throws sentinel messages 'ai_busy' / 'ai_region' for friendly frontend text. */
async function callGemini(env, { contents, system, search, temp, model }) {
  ...
  const primary = model || env.GEMINI_MODEL || 'gemini-2.5-flash';
  ...
  const plan = (fallback && fallback !== primary) ? [primary, primary, fallback] : [primary, primary];
  for (let i = 0; i < plan.length; i++) {
    const res = await fetch(`${baseUrl}/v1beta/models/${plan[i]}:generateContent?key=${geminiKey}`, {
      method: 'POST',
    ...
    if (res.status === 429 || res.status === 503) {
      if (i < plan.length - 1) { await new Promise(r => setTimeout(r, 350 + Math.random() * 400)); continue; }
      throw new Error('ai_busy');
    }
    if (res.status === 400 && /location|FAILED_PRECONDITION/i.test(t)) throw new Error('ai_region');
    throw new Error(`AI upstream error ${res.status}: ${t.slice(0, 160)}`);
  }
  throw new Error('ai_busy');
}

/** Daily-quota bump that never breaks the request if the usage table is missing (pre-migration). */
async function bumpQuota(env, key, amount, limit) {
  try {
    const day = new Date().toISOString().slice(0, 10);
    const row = await env.DB.prepare(`
      INSERT INTO usage (uid, day, count) VALUES (?, ?, ?)
      ON CONFLICT(uid, day) DO UPDATE SET count = count + ?
      RETURNING count
    `).bind(key, day, amount, amount).first();
    return !(row && row.count > limit);
  } catch (e) {
    return true; // usage table not migrated yet — allow rather than break (run migration 0003)
  }
}
...
/* ===== Shop Items via Gemini ===== */
async function getGeminiShop(env) {
  const prompt = `คุณคือผู้เชี่ยวชาญด้านอะไหล่แต่งและอะไหล่ทดแทนรถยนต์ จัดทำรายการสินค้าแนะนำ 6-10 รายการที่เป็นที่นิยมระดับพรีเมียม (เช่น โช้คอัพ, ล้อแม็ก, เบรกเซรามิก, ของเหลวเกรด Racing)
ตอบเป็น JSON array เท่านั้น ห้ามเขียนคำนำ คำอธิบาย หรือ markdown ใดๆ นอกจาก JSON
แต่ละรายการต้องมี:
1. title: ชื่อสินค้า เช่น "Öhlins Road & Track Coilover"
2. shortDescription: สรุปสั้นๆ ว่าดียังไง
3. price: ราคาโดยประมาณ เช่น "฿85,000" หรือ "฿2,500 - ฿4,000"
4. make: ยี่ห้อรถที่ใส่ได้ เช่น "Honda", "Toyota", "BMW" หรือ "ทุกยี่ห้อ"
5. model: รุ่นรถที่รองรับ เช่น "Civic", "Camry", "Series 3" หรือ "ทุกรุ่น"
6. type: ประเภท เช่น "ช่วงล่าง", "เบรก", "เครื่องยนต์", "ของเหลว", "อุปกรณ์เสริม"

ตัวอย่าง JSON:
[
  { "title": "Brembo GT 6-Pot", "shortDescription": "ระบบเบรกประสิทธิภาพสูง หยุดมั่นใจทุกย่านความเร็ว", "price": "฿120,000", "make": "ทุกยี่ห้อ", "model": "ทุกรุ่น", "type": "เบรก" }
]`;
  const text = await callGemini(env, {
    contents: [{ role: 'user', parts: [{ text: prompt }] }],
    temp: 0.5,
...
async function fetchAndSaveShop(env) {
  if (!env.DB) throw new Error('D1 Database connection is not configured');
  const items = await getGeminiShop(env);
  if (!Array.isArray(items) || items.length === 0) throw new Error('Fetched shop array is empty');

  await env.DB.prepare('DELETE FROM shop_items').run();
  const stmt = env.DB.prepare(
    'INSERT INTO shop_items (title, short_description, price, make, model, image_url, type, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)'
  );
  const now = Date.now();
  await env.DB.batch(items.map(n => stmt.bind(
    n.title || '', n.shortDescription || n.short_description || '', n.price || '',
    n.make || '', n.model || '',
    'premium_part.png', 
    n.type || 'อุปกรณ์เสริม', now
  )));
}
    ...
    try {
      ...
      if (url.pathname === '/api/login' && request.method === 'POST') {
        ...
        try {
          ...
          // created_at/banned columns not migrated yet — legacy upsert so login keeps working
          await env.DB.prepare(`
            ...
            ON CONFLICT(uid) DO UPDATE SET
              ...
              last_login = excluded.last_login,
              role = CASE WHEN excluded.role = 'owner' THEN 'owner' ELSE users.role END
          `).bind(uid, name, email, photo, isOwner ? 'owner' : 'user', now).run();
        }

        let row = null;
        try {
          row = await env.DB.prepare('SELECT role, banned FROM users WHERE uid = ?').bind(uid).first();
        } catch (e) {
          try { row = await env.DB.prepare('SELECT role FROM users WHERE uid = ?').bind(uid).first(); } catch (e2) {}
        }
      ...
      if (url.pathname === '/api/ai/chat' && request.method === 'POST') {
        return await guarded('user', async (actor) => {
          ...
          if (rank(actor.role) < rank('admin')) {
            const ok = await bumpQuota(env, actor.payload.sub, 1, parseInt(env.AI_DAILY_LIMIT || '60', 10));
            if (!ok) return deny('quota', 429);
          }
      ...
      if (url.pathname === '/api/ai/live-token' && request.method === 'POST') {
        return await guarded('user', async (actor) => {
          ...
          if (rank(actor.role) < rank('admin')) {
            const ok = await bumpQuota(env, actor.payload.sub, 5, parseInt(env.AI_DAILY_LIMIT || '60', 10));
            if (!ok) return deny('quota', 429);
          }
          ...
          const liveModel = env.GEMINI_LIVE_MODEL || 'gemini-2.5-flash-native-audio-latest';
          const baseUrl = env.GEMINI_BASE_URL || 'https://generativelanguage.googleapis.com';
          const now = Date.now();
      ...
      if (url.pathname === '/api/diagnose' && request.method === 'POST') {
        try {
          ...
          if (!exempt) {
            ...
            const ok = await bumpQuota(env, key, 1, limit);
            if (!ok) return deny('quota', 429);
          }
...
${convo}`;
      ...
      if (url.pathname === '/api/shop' && request.method === 'GET') {
        ...
        let query = 'SELECT * FROM shop_items ORDER BY created_at DESC LIMIT 50';
        let params = [];
        if (make && model) {
           query = 'SELECT * FROM shop_items WHERE (make = ? OR make = ?) AND (model = ? OR model = ?) ORDER BY created_at DESC LIMIT 50';
           params = [make, 'ทุกยี่ห้อ', model, 'ทุกรุ่น'];
        }
      ...
      if (url.pathname === '/api/magazine/sync' && request.method === 'POST') {
        return await guarded('moderator', async (actor) => {
          ...
          await addAudit(env, actor, 'magazine.sync', 'Magazine synced with AI');
          return json({ success: true });
        })();
      }

      /* ===== SHOP MANAGEMENT (moderator+) ===== */
      if (url.pathname === '/api/shop/sync' && request.method === 'POST') {
        return await guarded('moderator', async (actor) => {
          await fetchAndSaveShop(env);
          await addAudit(env, actor, 'magazine.sync', 'Shop synced with AI');
          return json({ success: true });
        })();
      }
